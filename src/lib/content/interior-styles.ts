...
// The listing, the home carousel and every style page read the same twelve rows, so one
// cached query serves all of them rather than each fetching what it needs.
export const findInteriorStyles = cache(async (locale: Locale): Promise<InteriorStyleT[]> => {
  ...
  return docs.map(toInteriorStyle)
})
