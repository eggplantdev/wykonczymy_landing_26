...
| — local dev              | Docker, `postgres:18-alpine`   | Pinned deliberately — dev/prod parity, and `pg_dump` refuses a server newer than itself, so this must track Neon's major. **Port 5436**, see below          |
