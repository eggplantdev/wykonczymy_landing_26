...
`/oferta` and a known-missing path returned `429` (LiteSpeed rate limiting) before the
no-trailing-slash and 404 behaviour could be checked. Re-run those two probes, spaced out, before
relying on either.
